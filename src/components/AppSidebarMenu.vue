...
<template>
	<div class="app-sidebar-menu overflow-hidden">
		<div v-if="store.open && heading" class="border-b border-[#202932] mb-1">
			<p class="px-4 font-medium  pt-[10px] text-brand-lightGrey text- mb-2">{{heading}}</p>
		</div>
		<ul class="list-none">
			<li v-for="item in items" :key="item.id">
					...
					<Disclosure v-slot="{ open }">
						...
						>
							<div class="flex items-center">
								<i :class="[store.open ? 'mr-4' : '', item.icon]" />
				...
				<RouterLink
					v-else
					:to="
						item.children ? item.children[item.children.length - 1].to : item.to
					"
					v-tippy="{
					...
					:class="[
						'text-brand-lightGrey app-sidebar-menu-item py-2.5 relative cursor-pointer flex items-center text-brand-lightGrey',
