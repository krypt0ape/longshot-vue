...
<template>
  <div class="blog-post-preview overflow-hidden">
    ...
    <div class="p-[30px]">
      <h2 class="text-white text-3xl font-medium mb-4">
        {{ post.fields.title }}
      </h2>
      <p class="text-brand-darkerGrey mb-2">{{ post.fields.shortDescription }}</p>
      <div class="ml-2 mt-6 flex justify-between">
        <NuxtLinkLocale class="font-medium text-brand-darkerGrey hover:text-brand-lightGrey"
          :to="`/blog/${post.fields.slug}`">
          <i class="fa-solid fa-angle-right mr-4" />Read More
        </NuxtLinkLocale>
        <NuxtLinkLocale :to="'/blog/category/' + post.fields.postCategory.fields.slug"
          class="flex space-x-4  text-brand-darkerGrey cursor-pointer hover:text-white transition">
