...
<template>
  <div id="sidebar-pusher" class="hidden relative overflow-hidden md:block"
    ...
    <div id="sidebar-wrapper" :class="[
      ...
      <div class="fixed top-0 left-0 right-0 bg-brand-bodyBg sidebar-top h-[70px] flex items-center z-20"
        :class="[store.open ? 'max-w-[250px]' : 'max-w-[70px]']">
        <div v-if="store.open" class="w-[250px] bg-brand-bodyBg items-center justify-between px-4 transition flex">
          <div class="cursor-pointer text-xl flex items-center justify-center" @click="store.toggle">
            <i class="fa-solid fa-bars text-white" />
          </div>

          <PrimaryButton to="/sports/home" as="RouterLink" class="!px-[15px] py-[14px]" v-if="sportsActive">Sports
          </PrimaryButton>
          <NeutralButton to="/sports/home" as="RouterLink" class="!px-[15px] py-[14px]" v-else>Sports</NeutralButton>

          <SecondaryButton to="/casino/home" as="RouterLink" class="!px-[15px] py-[14px]" v-if="casinoActive">Casino
          </SecondaryButton>
          <NeutralButton to="/casino/home" as="RouterLink" class="!px-[15px] py-[14px]" v-else>Casino</NeutralButton>
        </div>
      ...
      <div id="sidebar" :class="[store.open ? 'p-[16px]' : 'p-[10px]', 'relative  ']">
        <div v-if="!store.open" class="text-center mt-2">
          <PrimaryButton as="RouterLink" to="/sports/home" small class="mb-3 !rounded-xl"
            v-tippy="{ content: 'Sports', placement: 'right' }">
            <i class="fa-solid fa-futbol text-white text-2xl py-1.5" />
          </PrimaryButton>
          <SecondaryButton as="RouterLink" to="/casino/home" small class="mb-3 !rounded-xl"
