...
const tabBorder = computed(() => {
  if (props.item.active) {
    return props.color === "primary"
      ? "background: linear-gradient(180deg, #38A433 0%, #0e1725 100%);"
      : "background: linear-gradient(180deg, #FA00FF 0%, #0e1725 100%);";
  }
...
const tabBackground = computed(() => {
  if (props.item.active) {
    return props.color === "primary"
      ? "background: linear-gradient(180deg, #203e33 0%, #0e1725 100%);"
      : "background: linear-gradient(180deg, #6A2B74 0%, #0e1725 100%);";
  }

  return "background: linear-gradient(180deg, #182330 0%, #0e1725 100%);";
});
</script>
<template>
  <RouterLink :to="item.to ?? route.path" :scrollToTop="false" @click="item.action" :disabled="disabled">
    ...
    'absolute  z-10 top-0 mx-auto transition ease-in duration-100  group-hover:translate-y-0 ',
          ...
          <g filter="url(#filter0_f_186_1312)">
            <ellipse cx="99.5" cy="-7.48156" rx="77.5" ry="21.5184" :fill="svgFill" fill-opacity="0.36" />
          </g>
          <defs>
            <filter id="filter0_f_186_1312" x="0.481562" y="-50.5184" width="198.037" height="86.0737"
              filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
              <feFlood flood-opacity="0" result="BackgroundImageFix" />
              <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape" />
              <feGaussianBlur stdDeviation="10.7592" result="effect1_foregroundBlur_186_1312" />
            </filter>
          </defs>
        </svg>
